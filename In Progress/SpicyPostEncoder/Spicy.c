...
#pragma config(Sensor, in1,    AutoPot,        sensorNone)
...
#pragma config(Sensor, in8,    Lift_Pot,       sensorPotentiometer)
#pragma config(Sensor, dgtl3,  Climb_Switch,   sensorTouch)
#pragma config(Sensor, dgtl4,  Drive_Encoder,  sensorQuadEncoder)
#pragma config(Sensor, dgtl12, Lift_Switch,    sensorTouch)
#pragma config(Motor,  port1,           RightClaw,     tmotorVex393HighSpeed_HBridge, openLoop)
#pragma config(Motor,  port2,           Lift_C,        tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port3,           Lift_B,        tmotorVex393HighSpeed_MC29, openLoop, reversed)
...
#pragma config(Motor,  port8,           Lift_A,        tmotorVex393HighSpeed_MC29, openLoop)
#pragma config(Motor,  port9,           Lift_D,        tmotorVex393HighSpeed_MC29, openLoop, reversed)
...
#define Lift_UpperLimit 850
#define Lift_Pos1 2960
#define Lift_Pos2 1860
#define Lift_Pos3 850
#define Lift_PosAuto 850
...
#define RightClaw_Open 1760
#define RightClaw_Closed 3230
#define RightClaw_Mid 2460

volatile int RightClaw_Position = 0;
volatile int RightClaw_Power;

#define LeftClaw_Cross 4000
#define LeftClaw_Open 2160
#define LeftClaw_Closed 3765
#define LeftClaw_Mid 2900
...
task ProgramChooser() {
	...
	while (!picked) {
		...
		switch (auto) {
		case 0: autoName = "[Unibrow Stars]"; break;
		case 1: autoName = "[UnibrowStarCube]"; break;
		case 2: autoName = "[Prog Skills]"; break;
		case 3: autoName = "[CubeLeft]"; break;
		case 4: autoName = "[CubeRight]"; break;
...
/*void Aditya2 () { // Starts from back, rotates, picks stars and dumps on left fence INCOMPLETE
float turnErrorFactor = 1.35;
Lift_PositionCount = 1;
wait1Msec(250);
ClawPos = Open;
wait1Msec(250);
SetDriveControl(Rotation, 90 * turnErrorFactor, 1);
wait1Msec(250);
SetDriveControl(Line, 60, 2);
wait1MSec(250);
ClawPos = Closed;
wait1Msec(250);
SetDriveControl(Rotation, 90 * turnErrorFactor, 1);
wait1Msec(250);
SetDriveControl(Line, 37, 1);
wait1Msec(250);
...
task autonomous(){
	...
	switch(auto) {
	case 0: Aditya2(); break;
	case 1: Aditya1(); break;
	case 2: Paarth(); break;
	default: Nada(); break;
}
}
...
task usercontrol(){
	...
	int LeftDrive; int RightDrive;int TankDrivePower;
	int arcadeTurnConst = 0.9;
	ClawPos = 3;
	clearSounds();
	while (true){
		//DisplayAuto();

	//////////////////DRIVE FUNCTION(ARCADE)///////////////////
	TankDrivePower = abs(vexRT(Ch3)) > 20 ? vexRT(Ch3) : 0;

	if(TankDrivePower > 0)
			TankDrivePower = TrueSpeed[TankDrivePower];
		else if(TankDrivePower < 0)
			TankDrivePower = -TrueSpeed[-TankDrivePower];
		else
			TankDrivePower = 0;
	...
	motor[RightDriveA] = vexRT[Ch1] > 100 || vexRT[Ch1] < -100 ? (TankDrivePower*arcadeTurnConst) -vexRT[Ch1] : TankDrivePower-vexRT[Ch1];
	motor[RightDriveB] = vexRT[Ch1] > 100 || vexRT[Ch1] < -100 ? (TankDrivePower*arcadeTurnConst) -vexRT[Ch1] : TankDrivePower-vexRT[Ch1];
	motor[LeftDriveA] = vexRT[Ch1] > 100 || vexRT[Ch1] < -100 ? (TankDrivePower*arcadeTurnConst) +vexRT[Ch1] : TankDrivePower+vexRT[Ch1];
	motor[LeftDriveB] = vexRT[Ch1] > 100 || vexRT[Ch1] < -100 ? (TankDrivePower*arcadeTurnConst) +vexRT[Ch1] : TankDrivePower+vexRT[Ch1];
...
//if(SensorValue[Lift_Encoder] >= Lift_PosRelease){
//while(bSoundActive && bSoundQueueAvailable){
//  //	playSoundFile("Yee.wav");
//    playImmediateTone(440, 500);
//}
//}
...
task liftcontrol(){
	int Lift_Current; float LkP = .2; int Lift_Hold = 0; int LP;int Kd = 0.02;int error; int lasterror; int Deri;
	while (true){

		Lift_Current = SensorValue(Lift_Pot);
		LP = -1 * (Lift_Position - Lift_Current);

		Deri = lasterror-error;
		lasterror=error;
    ...
    break;
		...
		SetLiftMotors(Lift_Power+Deri*Kd);
		wait1Msec(20);
	}
}
